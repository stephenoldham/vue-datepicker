...
var cjs = deepmerge_1;function getBoundingClientRect(element) {
...
}var defaultModifiers = [eventListeners, popperOffsets$1, computeStyles$1, applyStyles$1];var hash = {
  left: 'right',
  right: 'left',
  bottom: 'top',
  top: 'bottom'
...
};var dayjsPlugins = [isoWeek, updateLocale, isBetween, advancedFormat, quarterOfYear, localeData].map(function (plugin) {
  return dayjs_min.extend(plugin);
});
var createPopper = popperGenerator({
  defaultModifiers: [].concat(_toConsumableArray(defaultModifiers), [flip$1, preventOverflow$1, offset$1, arrow$1])
});
var script = {
  name: 'Datepicker',
  props: {
  ...
  computed: {
    selectedReadable: function selectedReadable() {
      var _this = this;
      ...
      if (this.type == 'range') {
        var dates = [];
        this.selected.map(function (date) {
          dates.push(date.format(_this.format));
        });
        return dates.join(' - ');
      }
    ...
    weekStartsOn: function weekStartsOn() {
      return this.opts.weekStartsOn;
    },
    weekEndsOn: function weekEndsOn() {
    ...
    weekDays: function weekDays() {
      var dow = this.opts.weekStartsOn;
      var days = [];
    ...
    days: function days() {
      var days = [];
      var monthStart = this.focus.startOf('month'),
      ...
      var monthEnd = this.focus.endOf('month'),
      ...
      var start = monthStart.subtract(subDays, 'day'),
    ...
    months: function months() {
      var months = [];
      var start = this.focus.startOf('year');
    ...
    years: function years() {
      var years = [];
      var start = this.focus.subtract(4, 'years');
    ...
    canAddTime: function canAddTime() {
      var offset = this.view == 'years' ? 12 : 1,
    ...
    canSubTime: function canSubTime() {
      var offset = this.view == 'years' ? 12 : 1,
          unit = this.view == 'days' ? 'months' : 'years',
    ...
    // Range
    isSelectingRange: function isSelectingRange() {
      if (this.type !== 'range' || this.selected == null) return false;
      return this.selected.length == 1;
    },
    hasRangePresets: function hasRangePresets() {
      if (this.type !== "range") return false;
      return this.rangePresets.length > 0;
    },
    rangePresetDefaults: function rangePresetDefaults() {
      return {
        today: {
          label: 'Today',
          from: dayjs_min(),
    ...
    // Apply Button
    requireConfirm: function requireConfirm() {
    ...
    // Day styling
    dayEdgeOffset: function dayEdgeOffset() {
    ...
    _theme: function _theme() {
      ...
      var base = {
        ...
        day: {
          hover: "group-hover:bg-".concat(color, "-200"),
          selected: "bg-".concat(color, "-500 text-white"),
          weekendBg: 'bg-gray-100',
          inRange: "text-".concat(color, "-500 bg-").concat(color, "-300")
        },
        month: {
          ...
          tileBg: "bg-gray-100 hover:bg-".concat(color, "-500")
        },
        rangeEndHover: "group-hover:text-".concat(color, "-500 group-hover:bg-").concat(color, "-300"),
        rangePresets: {
          bg: 'bg-gray-100',
          normal: 'text-gray-500',
          hover: "hover:text-gray-900 hover:bg-".concat(color, "-200"),
          selected: "text-white bg-".concat(color, "-500")
        },
        applyBtn: {
          bg: this.canConfirm ? "bg-".concat(color, "-500") : "bg-".concat(color, "-200")
        }
      };
      var dark = {
        ...
        day: {
          hover: "group-hover:bg-".concat(color, "-900"),
          selected: "bg-".concat(color, "-500 text-white"),
          weekendBg: 'bg-gray-800 bg-opacity-50',
          inRange: "text-".concat(color, "-300 bg-").concat(color, "-700")
        },
        month: {
          current: 'border-gray-500 border hover:border-transparent',
          tileBg: "bg-gray-800 hover:bg-".concat(color, "-500")
        },
        rangeEndHover: "group-hover:text-".concat(color, "-300 group-hover:bg-").concat(color, "-700"),
        rangePresets: {
          bg: 'bg-gray-800 bg-opacity-50',
          normal: 'text-gray-500',
          hover: "hover:text-white hover:bg-".concat(color, "-900"),
          selected: "text-white bg-".concat(color, "-500")
        },
        applyBtn: {
          bg: this.canConfirm ? "bg-".concat(color, "-500") : "bg-".concat(color, "-900")
        }
      };
  ...
  watch: {
    ...
    setupComplete: function setupComplete(newValue) {
      if (this.debug) {
        console.log('setupComplete change: ' + newValue);
      }
    },
    isConfirmed: function isConfirmed(newValue) {
      if (this.debug) {
    ...
    selectedReadable: function selectedReadable(newValue, oldValue) {
      if (this.debug) {
  ...
  methods: {
    setupPickerOptions: function setupPickerOptions() {
    ...
    setupPopper: function setupPopper() {
      ...
      this.$nextTick(function () {
        var distance = _this4.opts.withPointer ? 12 : 4;
        var modifiers = [{
          name: 'preventOverflow',
          options: {
            padding: {
          ...
          name: 'flip',
          enabled: _this4.opts.alwaysInView,
          options: {
            padding: 5
          }
        }, {
          ...
          name: 'arrow',
          options: {
            enabled: _this4.opts.withPointer,
            padding: 8
          }
        }];
        _this4.popper = createPopper(_this4.$refs.trigger, _this4.$refs.picker, {
          placement: _this4.placement,
          modifiers: modifiers
        });
      });
    },
    teardownPopper: function teardownPopper() {
      if (this.popper !== undefined) {
        this.popper.destroy();
      }
    },
    setupPickerDateLimits: function setupPickerDateLimits() {
      // Set min/max date limits
      if (this.from !== null) {
        var from = dayjs_min(this.from);
        if (!from.isValid()) console.error("The Datepicker 'from' date is invalid");
        this.minDate = from;
      }

      if (this.to !== null) {
        var to = dayjs_min(this.to);
    ...
    setupPickerFocus: function setupPickerFocus() {
      var _this5 = this;

      var defaultDate = this.default;

      if (defaultDate !== null) {
        defaultDate = Array.isArray(defaultDate) ? defaultDate : [defaultDate];
        defaultDate = defaultDate.map(function (date) {
        ...
        defaultDate.forEach(function (date) {
          // Check date can be selected
          if (!_this5.isSelectable(date)) {
            return console.error('This date cannot be selected, it is outside the allowed range!');
          } // Select date


          if (_this5.type == 'range') {
            if (_this5.selected == null) {
              _this5.selected = [];
            }

            _this5.selected.push(date);
          } else {
            _this5.selected = date;
          }
        });

        if (this.requireConfirm) {
          this.$nextTick(function () {
            _this5.isConfirmed = true;
            _this5.hasChanged = false;
          });
        }
    ...
    setupRangePresets: function setupRangePresets() {
      var _this6 = this;

      if (this.opts.rangePresets.length === 0) return;
      this.opts.rangePresets.forEach(function (preset, index) {
        if (typeof preset === 'string' && _this6.rangePresetDefaults[preset]) {
          _this6.rangePresets.push(_this6.rangePresetDefaults[preset]);

          return;
        }

        if (_typeof(preset) === 'object') {
          _this6.rangePresets.push(preset);

          return;
        }
      });
    },
    setupPickerTemplate: function setupPickerTemplate() {
      // Check for a slot template
      if (this.$slots.default !== undefined) {
        var content = this.$refs.trigger.innerHTML; // Search for template vars
    ...
    parseTemplate: function parseTemplate() {
      ...
      if (this.templateMatches == null) {
        var reg = /%date(.*?)%/g;
        var matches = this.template.match(reg);
        this.templateMatches = matches;
      } // Get the original template


      var template = this.template; // Loop through the %date% matches and update the new template var

      this.templateMatches.map(function (match) {
        var parts = _this7.replaceAll(match, '%', '').split('|');

        var format = parts.length > 1 ? parts[1] : _this7.format;
        template = _this7.replaceAll(template, match, _this7.selected.format(format));
      }); // Update the slot
    ...
    updateDenyDates: function updateDenyDates() {
      var _this8 = this;
      ...
      if (Array.isArray(this.disable)) {
        dates = this.disable.map(function (date) {
          return dayjs_min(date, ['DD-MM-YYYY', 'DD/MM/YYYY', 'YYYY-MM-DD', 'YYYY/MM/DD']);
        });
      }

      if (typeof this.disable == 'function') {
        var from = this.focus.subtract(1, 'months').startOf('month');
        var to = this.focus.add(1, 'months').endOf('month');
        var days = [];
        ...
        dates = days.filter(function (date) {
          return _this8.disable(date);
        });
      }
    ...
    select: function select(date) {
      var selectOnly = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    revertSelection: function revertSelection() {
      ...
      this.$nextTick(function () {
        _this9.hasChanged = false;
        _this9.isConfirmed = false;
      });
    },
    selectedChange: function selectedChange(newValue) {
      this.date = newValue;
    ...
    deselect: function deselect(date) {
    ...
    isFocusable: function isFocusable(date) {
    ...
    isSelectable: function isSelectable(date) {
      ...
      if (this.denyDates !== null && this.denyDates.length > 0) {
        for (var i = 0; i < this.denyDates.length; i++) {
    ...
    isSelected: function isSelected(date) {
      var format = this.getIsSelectedFormat();

      if (this.type == 'range' && this.selected !== null) {
        return this.selected.filter(function (selected) {
    ...
    getIsSelectedFormat: function getIsSelectedFormat() {
      var format = 'DMYYYY';
    ...
    isEndOfWeek: function isEndOfWeek(day) {
    ...
    getTimeMethod: function getTimeMethod(prefix) {
      return prefix + (this.view == 'days' ? 'Month' : 'Year');
    },
    addTime: function addTime() {
      var num = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
      var method = this.getTimeMethod('add');
      num = this.view == 'years' ? 12 : 1;
      this[method](num);
    },
    subTime: function subTime() {
      var num = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
      var method = this.getTimeMethod('sub');
      num = this.view == 'years' ? 12 : 1;
      this[method](num);
    },
    // Range specific
    // -----------------------------
    inRange: function inRange(day) {
      if (this.type !== 'range' || this.selected == null) return false;
      var rangeStart = this.selected[0];
      var rangeEnd = this.isSelectingRange ? this.rangeHover : this.selected[1];
      if (rangeEnd == null) return false;
      return day.isBetween(rangeStart, rangeEnd, 'day', '()');
    },
    isRangeStart: function isRangeStart(day) {
    ...
    selectRangePreset: function selectRangePreset(preset) {
      var _this10 = this;
    ...
    isPresetSelected: function isPresetSelected(preset) {
    ...
    getPresetIcon: function getPresetIcon(preset) {
    ...
    replaceAll: function replaceAll(str, search, replacement) {
    ...
    setDate: function setDate(date, show) {
...
var __vue_render__ = function __vue_render__() {
  ...
  }, [!_vm.inline ? _vm._ssrNode("<div" + _vm._ssrClass(null, [this.opts.triggerClass]) + ">", "</div>", [_vm._t("default", [_c('input', {
    ref: "input",
  ...
  }) + "></div>" : "<!---->") + " " + (_vm.inline || _vm.show ? "<div" + _vm._ssrClass("flex items-stretch rounded-lg", {
    'text-gray-900 bg-white': !_vm.dark,
    'text-white bg-gray-900': _vm.dark,
    'border border-gray-200': !_vm.dark && _vm.inline
  }) + "><div" + _vm._ssrClass("flex-1 pt-2 pb-1", [_vm.hasRangePresets ? 'pl-1 pr-3' : 'px-1']) + "><div class=\"flex items-center font-bold mb-1\"><div" + _vm._ssrClass("flex-initial flex items-center justify-center w-8 h-8 rounded-full leading-none select-none", [!_vm.canSubTime ? 'opacity-50' : '', _vm.canSubTime ? 'cursor-pointer ' + _vm._theme.navHover : '']) + "><svg viewBox=\"0 0 20 20\" fill=\"currentColor\" class=\"w-4 h-4\"><path fill-rule=\"evenodd\" d=\"M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z\" clip-rule=\"evenodd\"></path></svg></div> <div class=\"flex-1 text-center\">" + (_vm.view == 'days' ? "<span" + _vm._ssrClass("p-1 cursor-pointer rounded-md", [_vm._theme.navHover]) + ">" + _vm._ssrEscape(_vm._s(_vm.focus.format('MMMM'))) + "</span>" : "<!---->") + " " + (_vm.view !== 'years' ? "<span" + _vm._ssrClass("p-1 -mx-1 cursor-pointer rounded-md", [_vm._theme.navHover]) + ">" + _vm._ssrEscape(_vm._s(_vm.focus.format('YYYY'))) + "</span>" : "<!---->") + " " + (_vm.view == 'years' ? "<span>" + _vm._ssrEscape("\n                            " + _vm._s(this.focus.subtract(4, 'years').format('YYYY')) + " - " + _vm._s(this.focus.add(7, 'years').format('YYYY')) + "\n                        ") + "</span>" : "<!---->") + "</div> <div" + _vm._ssrClass("flex-initial flex items-center justify-center w-8 h-8 rounded-full leading-none select-none", [!_vm.canAddTime ? 'opacity-50' : '', _vm.canAddTime ? 'cursor-pointer ' + _vm._theme.navHover : '']) + "><svg viewBox=\"0 0 20 20\" fill=\"currentColor\" class=\"w-4 h-4\"><path fill-rule=\"evenodd\" d=\"M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z\" clip-rule=\"evenodd\"></path></svg></div></div> " + (_vm.view == 'days' ? "<div><div class=\"flex uppercase py-2\">" + _vm._ssrList(_vm.weekDays, function (weekDay) {
    return "<div" + _vm._ssrClass("text-center text-xs font-semibold", [_vm._theme.weekDay]) + " style=\"width: 14.28%\">" + _vm._ssrEscape("\n                            " + _vm._s(weekDay) + "\n                        ") + "</div>";
  }) + "</div> <div class=\"flex flex-wrap\">" + _vm._ssrList(_vm.days, function (day, index) {
    return "<div" + _vm._ssrClass("flex items-center pt-1 group", _vm._([_vm.isSelectable(day) ? 'cursor-pointer' : '', !_vm.isSelectable(day) ? 'cursor-default' : '', _vm.isOverflow(day) && _vm.isSelectable(day) && !_vm.inFullRange(day) ? 'opacity-50' : '', !_vm.isSelectable(day) ? 'opacity-25 line-through' : '', _vm.inRange(day) ? 'range' : '', _vm.isWeekend(day) ? _vm._theme.day.weekendBg : '', // Weekend corner rounding
    index == 5 ? 'rounded-tl-md' : '', index == 6 ? 'rounded-tr-md' : '', index == _vm.days.length - 2 ? 'rounded-bl-md' : '', index == _vm.days.length - 1 ? 'rounded-br-md' : ''])) + " style=\"width: 14.28%\"><div class=\"flex w-full items-center justify-center\"><div" + _vm._ssrClass("flex items-center justify-center h-10 text-sm leading-none", [_vm.isSelected(day) ? _vm._theme.day.selected : '', !_vm.isSelected(day) && _vm.isSelectable(day) && !_vm.isRangeEnd(day) ? _vm._theme.day.hover : '', !_vm.isSelected(day) && _vm.isRangeEnd(day) ? _vm._theme.rangeEndHover : '', _vm.isToday(day) && !_vm.inFullRange(day) && !_vm.isSelected(day) ? 'border border-gray-400' : '', !_vm.inRange(day) ? 'w-10 rounded-full' : '', _vm.inRange(day) ? ['w-full', _vm._theme.day.inRange, _vm.isStartOfWeek(day) ? "box-content -ml-1 pl-1" : '', _vm.isEndOfWeek(day) ? "box-content -mr-" + _vm.dayEdgeOffset + " pr-" + _vm.dayEdgeOffset : ''] : '', _vm.isRangeStart(day) ? ['w-full rounded-l-full rounded-r-none border-0', _vm.isEndOfWeek(day) ? "box-content -mr-" + _vm.dayEdgeOffset + " pr-" + _vm.dayEdgeOffset : ''] : '', _vm.isRangeEnd(day) ? ['w-full rounded-r-full rounded-l-none border-0', _vm.isStartOfWeek(day) ? "box-content -ml-1 pl-1" : ''] : '']) + ">" + _vm._ssrEscape("\n                                    " + _vm._s(day.format('D')) + "\n                                ") + "</div></div></div>";
  }) + "</div></div>" : "<!---->") + " " + (_vm.view == 'months' ? "<div><div class=\"flex flex-wrap\">" + _vm._ssrList(_vm.months, function (month) {
